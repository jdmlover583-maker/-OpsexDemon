...
const GUILD_ID = '1488058062555189390';
...
class TitanBot extends Client {
  ...
  async start() {
    try {
      ...
      startupLog(
        `Registering slash commands for guild ${GUILD_ID}...`
      );
      ...
      server.on(
        ...
        error => {
          ...
          if (
            ...
            logger.warn(
              `Web server reported a duplicate bind warning ` +
              `on ${host}:${port}, but the bot remains online.`
            );
  ...
  async registerCommands() {
    ...
    try {
      ...
      startupLog(
        `Registering ${this.commands.size} ` +
        `commands to guild ${GUILD_ID}...`
      );
       ...
       * IMPORTANT:
       *
       * registerSlashCommands() is your existing command
       * loader. It controls how the commands are actually
       * sent to Discord.
       *
       * We pass the guild ID so the loader can register
       * commands specifically to your server.
       */
      ...
      startupLog(
        `✅ ${this.commands.size} commands registered ` +
        `to guild ${GUILD_ID}`
      );
